...
export const makeTestPrompter = (answers: Record<string, unknown> = {}): TestPrompter => {
  ...
  const prompter = new Prompter({
    ...
    info: (message) => Effect.sync(() => void log.infos.push(message)),
    message: (message) => Effect.sync(() => void log.messages.push(message)),
    ...
    text: (params: TextParams) => answer<string>(params.message),
    confirm: (params: ConfirmParams) => answer<boolean>(params.message),
